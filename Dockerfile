...
RUN pip install -q torch==2.4.0+cu121 torchvision==0.19.0+cu121 torchaudio==2.4.0+cu121 torchtext==0.18.0 torchdata==0.8.0 --extra-index-url https://download.pytorch.org/whl/cu121 \
    tqdm==4.66.5 numpy==1.26.3 imageio==2.35.1 imageio-ffmpeg==0.5.1 xformers==0.0.27.post2 moviepy==1.0.3 transformers accelerate peft sentencepiece==0.2.0 pillow==9.5.0 runpod && \
    git clone -b DimensionX https://github.com/camenduru/diffusers && cd /content/diffusers && pip install -e . && \
    ...
    aria2c --console-log-level=error -c -x 16 -s 16 -k 1M https://huggingface.co/camenduru/DimensionX/resolve/main/orbit_left_lora_weights.safetensors -d /content/lora -o orbit_left_lora_weights.safetensors
